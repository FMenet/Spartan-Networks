...
from AttackModel import cnn_model,spartan_network, unprotected_network
from cleverhans.utils_keras import KerasModelWrapper

FLAGS = flags.FLAGS

keras_global_model = 'globvar'
...
def prep_bbox(sess, x, y, X_train, Y_train, X_test, Y_test,
    ...
    kerasmodel = spartan_network(sess=sess, train=True,w=3)
    global keras_global_model
    keras_global_model = kerasmodel
    model = KerasModelWrapper(kerasmodel)
    predictions = model(x)
    ...
    print(kerasmodel.get_weights(), kerasmodel.layers)
...
def mnist_blackbox(train_start=0, train_end=60000, test_start=0,
                   ...
                   nb_epochs_s=10, lmbda=0.1, epsilon=0.3):
    """
    ...
    # Initialize the Fast Gradient Sign Method (FGSM) attack object.
        fgsm_par = {'eps': epstep, 'ord': np.inf, 'clip_min': 0., 'clip_max': 1.}
    ...
    # Evaluate the accuracy of the "black-box" model on adversarial examples
        accuracy = model_eval(sess, x, y, model(x_adv_sub), X_test, Y_test,
                          args=eval_params)
        print('Test accuracy of oracle on BB Adversarial Samples with epsilon = %s : '%epstep + str(accuracy))
        if pyp :
            ...
            for j in range(len(y_adv_np)-1):
                print(str(y_adv_np[j][3])+"predit, et le reel etait : "+str(np.argmax(y_adv_np[j][1])))
...
def main(argv=None):
    print(mnist_blackbox(nb_classes=FLAGS.nb_classes, batch_size=FLAGS.batch_size,
                         learning_rate=FLAGS.learning_rate,
                         nb_epochs=FLAGS.nb_epochs, holdout=FLAGS.holdout,
                         data_aug=FLAGS.data_aug, nb_epochs_s=FLAGS.nb_epochs_s,
                         lmbda=FLAGS.lmbda, epsilon=FLAGS.epsilon))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float('epsilon', 0.03, 'steps, defaut 0.03')
